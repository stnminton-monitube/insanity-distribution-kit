...
async function queueOnRelay(relayUrl, relayToken, payload) {
  ...
  let parsed;
  try { parsed = new URL(base); } catch (e) { throw new Error('The render farm relay URL is not a valid web address.'); }
  if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('The render farm relay URL must start with http:// or https://.');
  let res;
  try {
    res = await fetch(`${base}/api/extension/renderfarm/queue`, {
      method: 'POST',
      headers: {
    ...
    if (e && (e.name === 'TimeoutError' || e.name === 'AbortError')) {
      throw new Error('The render farm relay did not respond within 30 seconds. Check the relay URL and your connection, then retry.');
    }
    throw new Error('Could not reach the render farm relay: ' + (e && e.message ? e.message : e));
  }
  let json = {};
...
function modeAfterTextCheck(mode, skipped) {
  return skipped && mode === 'both' ? 'texted' : mode;
}
...
async function sendToFarm(aepPath, settings, mode, format, onProgress) {
  if (!['texted', 'textless', 'both'].includes(mode)) throw new Error(`Unknown render mode: ${mode}`);
  let queueMode = mode;
  if (mode === 'textless' || mode === 'both') {
    onProgress && onProgress('checking for text layers…');
    const result = await disableTextAndSave(aepPath);
    if (result.skipped && mode === 'textless') return { aep: aepPath, skipped: true };
    // "Both" still owes the texted render even when there is no text-off
    // variant to create. Previously this returned early and queued nothing.
    queueMode = modeAfterTextCheck(mode, result.skipped);
    onProgress && onProgress(result.skipped
      ? 'no text layers found — queuing the texted render only…'
      : `${result.layersDisabled} text layer(s) disabled…`);
  }
  ...
  return { aep: aepPath, queued: true, row: result.row, mode: queueMode,
    note: queueMode !== mode ? 'No text layers found; queued the texted render only.' : '' };
}
...
async function runFarmBatch(aepPaths, settings, mode, format, onProgress) {
  if (!Array.isArray(aepPaths) || !aepPaths.length) return [];
