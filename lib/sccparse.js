...
function parseCaptions(filePath) {
  ...
  throw new Error('Use an .scc, .srt, or .vtt caption file.');
}
...
function retimeCaptionFile(srcPath) {
  if (!/\.(scc|srt|vtt)$/i.test(srcPath)) throw new Error('Use an .scc, .srt, or .vtt caption file.');
  ...
  const dest = versionedPath(srcPath.slice(0, -ext.length) + ' MASTER-TC' + ext);
