...
// House standard is 29.97 (see DELIVERABLES.md). The other rates are valid in
// the wider spec, so they warn rather than fail.
const HOUSE_FPS = '30000/1001';
const ALLOWED_FPS = [
  { r: '24000/1001', label: '23.976' },
  { r: '30000/1001', label: '29.97' },
  { r: '60000/1001', label: '59.94' },
];
...
async function runQC(file) {
  ...
  const isUHD = v.width === 3840 && v.height === 2160;
  const isHD = v.width === 1920 && v.height === 1080;
  if (isUHD || isHD) {
    results.push(item('pass', 'Resolution', `${res} (${isUHD ? 'Ultra HD' : 'Full HD'})`));
  } else {
    results.push(item('fail', 'Resolution', `${res} — spec allows only 3840x2160 or 1920x1080.`,
      'Set your Premiere sequence/export to 3840x2160 or 1920x1080.'));
  }
  ...
  if (v.codec_name === 'prores') {
    ...
    if (isUHD) {
      if (prof.includes('XQ')) results.push(item('pass', 'Codec', `Apple ProRes 4444 XQ (profile: ${v.profile})`));
      else results.push(item('fail', 'Codec profile', `ProRes profile is "${v.profile}" — UHD requires ProRes 4444 XQ.`,
        'In Premiere export settings choose Apple ProRes 4444 XQ.'));
    } else {
      if (prof === 'HQ' || prof.includes('422 HQ')) results.push(item('pass', 'Codec', `Apple ProRes 422 HQ`));
      else results.push(item('fail', 'Codec profile', `ProRes profile is "${v.profile}" — Full HD requires ProRes 422 HQ.`,
    ...
    results.push(item('fail', 'Codec', `Video codec is "${v.codec_name}" — spec requires Apple ProRes.`,
      'Re-export from Premiere: Format = QuickTime, Codec = Apple ProRes 422 HQ (HD) or 4444 XQ (UHD).'));
  }

  // ---- Bit depth / chroma ----
  const pix = v.pix_fmt || '';
  if (isUHD) {
    if (pix === 'yuv444p12le') results.push(item('pass', 'Color sampling / bit depth', '4:4:4, 12-bit'));
    else results.push(item('warn', 'Color sampling / bit depth', `Pixel format is ${pix} — UHD spec is 4:4:4 12-bit (no alpha).`,
      'ProRes 4444 XQ export normally produces this automatically. Make sure "no alpha" is selected.'));
  } else if (isHD) {
  ...
  // ---- Frame rate ----
  const fpsMatch = ALLOWED_FPS.find(f => f.r === v.r_frame_rate);
  if (fpsMatch && v.r_frame_rate === HOUSE_FPS) {
    results.push(item('pass', 'Frame rate', '29.97 fps (house standard)'));
  } else if (fpsMatch) {
    results.push(item('warn', 'Frame rate', `${fpsMatch.label} fps — valid in the wider spec, but the house standard is 29.97.`,
      'Confirm the distributor accepts this rate, or re-export at 29.97.'));
  }
  else results.push(item('fail', 'Frame rate', `${v.r_frame_rate} — spec allows 23.976, 29.97 or 59.94.`,
    'Set sequence/export frame rate to 23.976, 29.97 or 59.94.'));
