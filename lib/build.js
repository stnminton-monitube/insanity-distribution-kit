...
async function buildMaster(opts) {
  ...
  if (v.codec_name !== 'prores' || !/(^HQ$|422 HQ)/i.test(v.profile || '')) throw new Error(`Video must be Apple ProRes 422 HQ before building the broadcast master (found ${v.codec_name || 'unknown'} ${v.profile || ''}).`);
