...
async function api(token, method, path, body) {
  ...
  try {
    ...
    if (e && (e.name === 'TimeoutError' || e.name === 'AbortError')) {
      throw new Error('Notion did not respond within 30 seconds. Check your connection and retry.');
    }
    throw new Error('Could not reach Notion: ' + (e && e.message ? e.message : e));
  }
  let json = {};
  try { json = await res.json(); } catch (e) { /* preserve the HTTP status below */ }
...
async function listEpisodes(token, dbId) {
  ...
  do {
    ...
    for (const p of r.results) {
      ...
      if (!title.trim() || title.startsWith('📋 TEMPLATE')) continue;
      out.push({
        id: p.id,
        url: p.url,
        title,
...
async function createEpisode(token, dbId, fields) {
  ...
  if (!title) throw new Error('Enter an episode title before creating the episode.');
  const properties = {
    Episode: { title: [{ text: { content: title.slice(0, 1900) } }] },
...
async function createStageChecklist(token, pageId, { description, specLines, stages }) {
  const para = content => ({ object: 'block', type: 'paragraph', paragraph: { rich_text: [{ text: { content: String(content).slice(0, 1900) } }] } });
  const children = [
    para(description),
    {
