...
async function disableTextAndSave(aepPath) {
  ...
  if (fs.existsSync(outAepPath)) {
    throw new Error(`${path.basename(outAepPath)} already exists. Move or rename that older textless copy, then retry; it will not be overwritten.`);
  }
  // The relay derives this exact sibling name from the texted AEP path. A
  // version-suffixed copy would be safe locally but invisible to the farm, so
  // stop and make the user resolve the stale copy rather than overwrite it or
  // silently queue the wrong project.
  if (fs.existsSync(outAepPath)) {
    throw new Error(`A textless copy already exists: ${outAepPath} — move or rename that existing copy after confirming it is no longer needed, then retry.`);
  }
