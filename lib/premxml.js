...
function compilePattern(patternStr) {
  ...
  try { return new RegExp(patternStr, 'i'); }
  catch (e) { throw new Error(`The graphics name pattern is invalid: ${e.message}`); }
}
    ...
    for (const kindTag of ['video', 'audio']) {
      ...
      for (const trk of topBlocks(xml, 'track', kb[0].inner[0], kb[0].inner[1])) {
        ...
        for (const ci of topBlocks(xml, 'clipitem', trk.inner[0], trk.inner[1])) {
          const clipEnabled = firstValue(xml, 'enabled', ci.inner[0], ci.inner[1]);
          if (clipEnabled && clipEnabled.toUpperCase() === 'FALSE') continue;
...
function parseSequenceXML(filePath) {
  ...
  let fileMatch;
  while ((fileMatch = fileRe.exec(xml))) {
    const id = (fileMatch[1].match(/\bid="([^"]+)"/i) || [])[1];
    if (!id || fileDefs.has(id)) continue;
    const body = fileMatch[2];
    const rawName = (body.match(/<name>([\s\S]*?)<\/name>/i) || [])[1];
    const rawPath = (body.match(/<pathurl>([\s\S]*?)<\/pathurl>/i) || [])[1];
    fileDefs.set(id, { name: rawName ? unesc(rawName) : '', path: rawPath ? unesc(rawPath) : '' });
  }
  ...
  const parseKind = (kindTag, kindLabel, afterOffset) => {
    ...
    const raw = topBlocks(xml, 'track', k0, k1).map(trk => {
      ...
      const trackHeaderEnd = clipBlocks.length ? clipBlocks[0].start : trk.inner[1];
      const trackName = unesc(firstValue(xml, 'name', trk.inner[0], trackHeaderEnd) || '');
      ...
      const mkClip = (name, start, end, nested, sourceName, sourcePath) => ({
        name, sourceName: sourceName || name, sourcePath: sourcePath || '', start, end, nested: !!nested,
      ...
      const clips = clipBlocks.flatMap(cb => {
        // A disabled timeline clip is not audible and must never become a
        // music cue (or a graphics-log entry). Whole-track state is handled
        // separately above; this is the common per-clip Premiere toggle.
        const clipEnabled = firstValue(xml, 'enabled', cb.inner[0], cb.inner[1]);
        if (clipEnabled && clipEnabled.toUpperCase() === 'FALSE') return [];
        const cName = unesc(firstValue(xml, 'name', cb.inner[0], cb.inner[1]) || 'clip');
        const clipText = xml.slice(cb.inner[0], cb.inner[1]);
        const fileId = ((clipText.match(/<file\b[^>]*\bid="([^"]+)"/i) || [])[1]);
        const fileDef = fileId ? fileDefs.get(fileId) : null;
        const inlinePath = unesc(firstValue(xml, 'pathurl', cb.inner[0], cb.inner[1]) || '');
        const inlineName = unesc(firstValue(xml, 'name', cb.inner[0], cb.inner[1]) || '');
        const sourcePath = inlinePath || (fileDef && fileDef.path) || '';
        let sourceName = (fileDef && fileDef.name) || inlineName || cName;
        if (sourcePath) {
          try { sourceName = decodeURIComponent(sourcePath).split('/').pop() || sourceName; } catch (e) { /* keep XML name */ }
        }
        ...
        if (kindLabel === 'A') {
          ...
          if (innerSeq.length) {
            ...
            return [mkClip(cName, start, end, true, sourceName, sourcePath)]; // nest found, no real audio inside — stay visible, don't drop it
          }
        }
        return [mkClip(cName, start, end, false, sourceName, sourcePath)];
      });
      const seen = new Set();
      ...
      return { clips: unique, sig: unique.map(c => c.name + '@' + c.start).join('|'), enabled: trackEnabled, trackName };
    });
    ...
    const tracks = logical.map((t, i) => ({
      ...
      label: `${t.trackName ? t.trackName + ' · ' : ''}${kindLabel === 'A' ? 'Audio' : 'Video'} track #${i + 1}${t.stereo ? ' ·st' : ''}${t.enabled === false ? ' (disabled)' : ''}`,
      timelineName: t.trackName || '',
...
function makeTextlessXML(xmlPath, patternStr) {
  ...
  const patternRe = compilePattern(patternStr);
  ...
  const outPath = versionedPath(xmlPath.replace(/\.xml$/i, '') + ' TEXTLESS.xml');
...
function makeCleanXML(xmlPath, patternStr) {
  ...
  const outPath = versionedPath(xmlPath.replace(/\.xml$/i, '') + ' CLEAN.xml');
