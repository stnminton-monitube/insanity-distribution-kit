...
async function buildStems(opts) {
  const { full, dx, mx, fx, narr, outDir, onProgress = () => {} } = opts;
...
async function embedMaster(opts) {
  ...
  args.push('-timecode', srcTC, '-map_chapters', '-1', '-c:v', 'copy',
    ...
    '-c:a', 'pcm_s24le');
  TRACKS.forEach((t, i) => {
    args.push(`-metadata:s:a:${i}`, `title=${t.cfg} ${t.asgn} — ${t.ch}`);
    args.push(`-metadata:s:a:${i}`, `handler_name=${t.cfg} ${t.asgn} — ${t.ch}`);
  });
...
// "N seconds" of broadcast timecode at an NTSC-pulldown rate (23.976/29.97/59.94,
// the only rates DELIVERABLES.md allows) is N * 1001/1000 real seconds — lib/build.js's
...
const NTSC_RATES = new Set(['24000/1001', '30000/1001', '60000/1001']);
...
async function padStems(opts) {
  const { stemsDir, outDir, originator = 'Production Company', frameRate = '30000/1001', onProgress = () => {} } = opts;
  fs.mkdirSync(outDir, { recursive: true });
